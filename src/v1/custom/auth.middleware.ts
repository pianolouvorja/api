 ...
 */
export const optionalAuth: MiddlewareHandler<CustomAuthEnv> = createMiddleware<CustomAuthEnv>(
  async (c, next) => {
  ...
  if (session) {
    c.set("user", { id_user: session.id_user, email: session.email, display_name: session.display_name });
  }
  await next();
});
 ...
 */
export const requireAuth: MiddlewareHandler<CustomAuthEnv> = createMiddleware<CustomAuthEnv>(
  async (c, next) => {
  const raw = c.req.header("authorization") ?? "";
  const token = raw.startsWith("Bearer ") ? raw.slice(7) : "";
